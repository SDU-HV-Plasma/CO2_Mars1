...
gas='CO2m4'; %Valid Options: He, He_N2, Ar, CR_Ar, CO2.
ne=1e17; %m-3
dt=1e-9; %sec
%% INITIALIZATION
...
elseif (strcmp(lower(gas),lower('CO2m4')))
    rates=@rates_CO2m4;
...
while(~get(h_but_end,'userdata'))
    ...
    if(rem(cont,1)==0 && cont>1)
        ...
        legend('单位电子能量沉积','弹性碰撞能量散失','电子密度变化引起的电子温度变化','反应化学能带来的能量变化','物质在壁上的损失带来的能量变化','location','EastOutside');
        xlabel('Time');
    ...
    [nsp,species,X,R,lambda,dTe,B,Y,dS_ape,dS_el,dS_epro,dS_rea,dS_wall]=rates(Te,P,n,area_volume,Tg,0,me,mg,e,S_abs,dt);
